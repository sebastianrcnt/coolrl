...
def extract_traversal_health(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "league_anchor_traversal_rate": normalize_value(row.get("league_anchor_traversal_rate")),
        "league_current_traversal_rate": normalize_value(row.get("league_current_traversal_rate")),
        "league_recent_traversal_rate": normalize_value(row.get("league_recent_traversal_rate")),
        "league_older_traversal_rate": normalize_value(row.get("league_older_traversal_rate")),
    }
...
def render_compact_stdout(summary: dict[str, Any], deltas: dict[str, dict[str, float]] | None) -> str:
    ...
    safe_opened_count_5 = average_metric(opponents, SAFE_OPPONENTS, "opened_colors_count_5")
    safe_opened_count_5_rate = safe_opened_count_5 / 100.0 if safe_opened_count_5 is not None else None
    return "\n".join(
        [
            "Lost Cities anchor_safe015 compact",
            ...
            f"anchor {format_rate(traversal.get('league_anchor_traversal_rate'))}, "
            ...
            f"- safe opened: mean {format_value(safe_opened)}, 5-color {format_rate(safe_opened_count_5_rate)}",
