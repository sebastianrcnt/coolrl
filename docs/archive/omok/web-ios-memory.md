...
- **JS heap이 flat함.** MCTS tree나 replay-buffer growth가 없음. C-side MCTS 메모리 문제 (참조: [`mcts-memory-incident.md`](mcts-memory-incident.md))는 브라우저가 아닌 호스트의 Python training process에 대한 것입니다 — 브라우저의 MCTS는 TS에서 살았고, 각 search마다 할당되고 깔끔하게 tear down되었습니다.
