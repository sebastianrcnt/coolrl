...
**카드 단계**: 크기 `2 × hand_size`

- 슬롯 `i`의 플레이: 액션 ID `2i`
- 슬롯 `i`의 폐기: 액션 ID `2i + 1`
...
- 덱 뽑기: `len(deck) > 0`
- 색상 `c` 폐기 그리기: `len(discards[c]) > 0` AND `c != 보류 중인_discarded_color`
...
- `train.py`: 행위자/학습자 구조, 짧은 수명의 FIFO 재생, 자체 재생
